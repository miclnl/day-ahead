...
class EnhancedWeatherIntegrator:
    ...
    def _calculate_panel_angle_factor(self, sun_elevation: float, sun_azimuth: float) -> float:
        """Calculate panel angle efficiency factor"""
        
        if sun_elevation <= 0:
            return 0.0
        
        # Simplified calculation - would be more complex for real implementation
        # This assumes optimal panel orientation
        
        # Azimuth factor (best when sun is south)
        azimuth_diff = abs(sun_azimuth - self.solar_azimuth)
        if azimuth_diff > 180:
            azimuth_diff = 360 - azimuth_diff
        azimuth_factor = max(0, 1 - (azimuth_diff / 90))
        
        # Elevation factor
        optimal_elevation = 90 - self.solar_tilt
        elevation_diff = abs(sun_elevation - optimal_elevation)
        elevation_factor = max(0, 1 - (elevation_diff / 90))
        
        return azimuth_factor * elevation_factor
