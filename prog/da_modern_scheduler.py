...
try:
    import apscheduler
    # Get version safely
    apscheduler_version = getattr(apscheduler, '__version__', 'unknown')
    print(f"APScheduler version found: {apscheduler_version}")
    ...
    logging.info(f"APScheduler {apscheduler_version} loaded successfully")
...
class ModernDaScheduler(DaBase):
    ...
    def _get_current_grid_power(self) -> float:
        """Get current grid power usage from database"""
        try:
            # Haal recente grid data op uit database
            latest_data = self.db_da.get_column_data('values', 'grid', limit=1)
            if not latest_data.empty and 'value' in latest_data.columns:
                return float(latest_data.iloc[-1]['value'])
            return 0.0
        except Exception as e:
            logging.debug(f"Kon geen grid power ophalen: {e}")
            return 0.0

    def _get_battery_soc(self) -> float:
        """Get current battery state of charge from database"""
        try:
            # Haal recente batterij SOC data op uit database
            latest_data = self.db_da.get_column_data('prognoses', 'soc', limit=1)
            if not latest_data.empty and 'value' in latest_data.columns:
                return float(latest_data.iloc[-1]['value'])
            return 50.0  # Fallback waarde
        except Exception as e:
            logging.debug(f"Kon geen batterij SOC ophalen: {e}")
            return 50.0
    ...
    def _check_ha_connectivity(self) -> bool:
        """Check Home Assistant connectivity"""
        try:
            # Test HA connection door een eenvoudige API call te maken
            if hasattr(self, 'ha_integration') and self.ha_integration:
                # Test of we verbinding kunnen maken met HA
                test_result = self.ha_integration.test_connection()
                return test_result.get('success', False)
            else:
                # Geen HA integratie geconfigureerd
                logging.debug("Geen HA integratie geconfigureerd")
